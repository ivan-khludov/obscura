...
func testClient(name, password string) domain.ClientConfig {
	return domain.ClientConfig{Name: name, Password: password, Enabled: true}
}
...
func parseHookFailOnSecond() func([]byte) (hysteria2.ProtocolData, error) {
	calls := 0
	return func(raw []byte) (hysteria2.ProtocolData, error) {
		calls++
		if calls >= 2 {
			return hysteria2.ProtocolData{}, errors.New("parse failed")
		}
		return hysteria2.ParseProtocolData(raw)
	}
}

type errPasswordContext struct {
	*testutil.BuildContext
}

func (c *errPasswordContext) GeneratePassword(int) (string, error) {
	return "", errors.New("password generation failed")
}

func newErrPasswordContext(dataDir string) *errPasswordContext {
	return &errPasswordContext{BuildContext: testutil.NewBuildContext(dataDir)}
}
