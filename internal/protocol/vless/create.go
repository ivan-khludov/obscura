...
// CreateOptions holds VLESS-specific create parameters.
...
// BuildProtocolData builds and validates VLESS protocol data.
...
// NeedsInitialClient reports whether VLESS requires an enabled client.
...
func createOptionsFromSpec(spec domain.CreateVPNSpec) (CreateOptions, error) {
	...
	case *CreateOptions:
		...
		return *opts, nil
	default:
		return CreateOptions{}, fmt.Errorf("vless create options have unexpected type %T", spec.ProtocolOptions)
	}
}
...
func applyTransport(data *ProtocolData, v CreateOptions) error {
	...
	default:
		return fmt.Errorf("unsupported vless transport %q", v.Transport)
	}
	return nil
}
