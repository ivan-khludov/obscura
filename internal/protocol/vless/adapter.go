// Package vless implements the VLESS inbound protocol adapter for sing-box.
package vless
...
const typeName = "vless"

// Adapter implements protocol.Protocol for sing-box VLESS inbound.
type Adapter struct{}
...
// DefaultListen returns default listen options for VLESS inbounds.
...
// ValidateVPN checks VPN and client configuration for VLESS inbounds.
...
func (a *Adapter) ValidateClient(client domain.ClientConfig) error {
	...
	if client.Username != "" {
		if err := ValidateClientFlow(client.Username); err != nil {
			return err
		}
	}
	return nil
...
func validateEnabledClients(data ProtocolData, clients []domain.ClientConfig) error {
	...
	if enabled == 0 {
		return errors.New("at least one enabled client is required for VLESS authentication")
	}
...
// RenderInbound builds a sing-box VLESS inbound map from VPN and client state.
...
// ClientURI returns a vless:// connection URI for the client.
